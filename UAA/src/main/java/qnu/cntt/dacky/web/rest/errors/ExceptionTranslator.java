package qnu.cntt.dacky.web.rest.errors;
...
public class ExceptionTranslator implements ProblemHandling, SecurityAdviceTrait {
	...
	@ExceptionHandler
	public ResponseEntity<Problem> handleEmailAlreadyUsedException(qnu.cntt.dacky.service.exception.EmailAlreadyUsedException ex,
	...
	public ResponseEntity<Problem> handleUsernameAlreadyUsedException(
			qnu.cntt.dacky.service.exception.UsernameAlreadyUsedException ex, NativeWebRequest request) {
	...
	@ExceptionHandler
	public ResponseEntity<Problem> handleInvalidPasswordException(qnu.cntt.dacky.service.exception.InvalidPasswordException ex,
			NativeWebRequest request) {
	...
	public ProblemBuilder prepare(final Throwable throwable, final StatusType status, final URI type) {
		...
		if (activeProfiles.contains(JHipsterConstants.SPRING_PROFILE_PRODUCTION)) {
			...
			if (throwable instanceof DataAccessException) {
				return Problem.builder().withType(type).withTitle(status.getReasonPhrase()).withStatus(status)
						.withDetail("Failure during data access").withCause(Optional.ofNullable(throwable.getCause())
								.filter(cause -> isCausalChainsEnabled()).map(this::toProblem).orElse(null));
			}
			if (containsPackageName(throwable.getMessage())) {
				return Problem.builder().withType(type).withTitle(status.getReasonPhrase()).withStatus(status)
						.withDetail("Unexpected runtime exception").withCause(Optional.ofNullable(throwable.getCause())
								.filter(cause -> isCausalChainsEnabled()).map(this::toProblem).orElse(null));
			}
		}
